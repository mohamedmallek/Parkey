 ...
 *
 * Utilise le fichier report-task.txt genere par le sonar-scanner CLI
 * (contient l'URL du serveur et l'id de la tache d'analyse cote
 * SonarQube), puis interroge l'API SonarQube :
...
// Selon l'outil utilise pour lancer l'analyse, report-task.txt n'atterrit
// pas toujours au meme endroit : sonar-scanner CLI (standalone, projets
// JS/TS) l'ecrit dans .scannerwork/, le plugin Maven dans target/sonar/.
const CANDIDATE_REPORT_TASK_FILES = [
  path.join(process.cwd(), '.scannerwork', 'report-task.txt'),
  path.join(process.cwd(), 'target', 'sonar', 'report-task.txt'),
];
const POLL_INTERVAL_MS = 5000;
const MAX_WAIT_MS = 5 * 60 * 1000;

function readReportTask() {
  const filePath = CANDIDATE_REPORT_TASK_FILES.find((candidate) => fs.existsSync(candidate));
  if (!filePath) {
    throw new Error(
      'Fichier report-task.txt introuvable (cherche dans : ' +
      CANDIDATE_REPORT_TASK_FILES.join(', ') +
      "). L'analyse SonarQube a-t-elle bien ete executee avant ce script ?"
    );
  }
